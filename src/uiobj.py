    ...
    try:
        ...
        if action_name == 'query_status':
            ...
            for info in info_list:
                if info['info'] == 'object':
                    s = '%d %d %d %d %d %.10f %.10f %.10f %.10f %d %d %d %d;' % (
                    ...
                    s = '%d %d %d %d %d;' % (
                        info_types.index(info['info']), info['time'], skill_types.index(info['type']), info['player'], info['target'])
                    ret_str_list.append(s)
...
class UIObject(threading.Thread):
    ...
    def push_queue_ui(self, obj: str):
        ...
        if act and act[0] == '_':
            self._game_obj.enqueue(timestamp, action.Action('{"action":"_platform","ai_id":%d}' % self._ai_id, act,
